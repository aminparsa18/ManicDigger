...
public class ServerSystemLoadConfig : ServerSystem
{
    ...
    public ServerSystemLoadConfig(ServerGameService server, IModEvents modEvents, ILanguageService languageService, IServerConfig serverConfig) : base(modEvents)
    {
        this.server = server;
    ...
    /// on subsequent ticks when <see cref="ServerGameService.ConfigNeedsSaving"/> is set.
    /// </summary>
    ...
    /// <see cref="ServerGameService._config"/> to <c>null</c> and writes a fresh default config.
    /// </summary>
    ...
    /// <summary>
    /// Serializes <see cref="ServerGameService._config"/> to <c>ServerConfig.txt</c> in the
    /// game config directory.
    /// <para>
    /// If <see cref="ServerGameService._config"/> is <c>null</c> (i.e. first run), the operator
