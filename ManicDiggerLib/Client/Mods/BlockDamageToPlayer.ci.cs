public class ModBlockDamageToPlayer : ModBase
{
    ...
    internal void UpdateBlockDamageToPlayer(Game game, float dt)
    {
        ...
        {
            block1 = game.map.GetBlock((int)(pX), (int)(pZ), (int)(pY));
        }
        if (game.map.IsValidPos(game.MathFloor(pX), game.MathFloor(pZ), game.MathFloor(pY) - 1))
        {
            block2 = game.map.GetBlock((int)(pX), (int)(pZ), (int)(pY) - 1);
        }
        ...
        //Player drowning
        int deltaTime = (int)(one * (game.platform.TimeMillisecondsFromStart() - game.lastOxygenTickMilliseconds)); //Time in milliseconds
        if (deltaTime >= 1000)
        {
            if (game.WaterSwimmingEyes())
            {
                ...
                if (game.PlayerStats.CurrentOxygen <= 0)
                {
                    ...
                    int dmg = (int)(one * game.PlayerStats.MaxHealth / 10);
                    if (dmg < 1)
                    {
                        dmg = 1;
                    }
