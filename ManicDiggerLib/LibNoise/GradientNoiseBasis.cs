...
/// <summary>
/// Alternative gradient noise basis that uses a fixed table of pre-computed
/// 3D unit vectors instead of a seed-built permutation table.
/// <para>
/// For each sample point, the eight corners of the surrounding unit cube are
/// hashed (using the corner coordinates and seed) to select a gradient vector
/// from <see cref="RandomVectors"/>. The dot product of each gradient with the
/// offset from that corner to the sample point is computed, and all eight
/// results are trilinearly interpolated to produce the final value.
/// </para>
/// <para>
/// Unlike <see cref="FastNoiseBasis"/>, the gradient table is independent of
/// the seed — only the hash mixes the seed in, so the same <see cref="RandomVectors"/>
/// array is reused for every instance.
/// </para>
/// </summary>
public class GradientNoiseBasis : Math
{
    ...
    /// <summary>
    /// 256 pre-computed 3D unit gradient vectors stored as flat XYZW groups
    /// (W is always 0 and unused — present for alignment only).
    /// Indexed as <c>RandomVectors[index * 4 + component]</c> where component
    /// is 0=X, 1=Y, 2=Z.
    /// </summary>
    ...
    /// given world position, using the fixed <see cref="RandomVectors"/> table
    /// and a hash of the integer grid coordinates mixed with <paramref name="seed"/>.
    /// <para>
    /// Algorithm:
    /// <list type="number">
    ///   <item>Floor the input to find the unit cube origin.</item>
    ///   <item>Smooth fractional offsets with an S-curve (quality-dependent).</item>
    ///   <item>For each of the 8 cube corners, hash the corner + seed to select
    ///         a gradient vector, then compute the dot product with the
    ///         offset from that corner to the sample point.</item>
    ///   <item>Trilinearly interpolate the 8 dot products.</item>
    /// </list>
    /// </para>
    /// </summary>
    ...
    /// <param name="seed">Seed value mixed into the corner hash.</param>
    /// <param name="noiseQuality">Interpolation smoothing level.</param>
    public double GradientCoherentNoise(double x, double y, double z,
        int seed, NoiseQuality noiseQuality)
    {
        // Integer cube origin (floor, handles negative coords correctly).
        int x0 = x > 0.0 ? (int)x : (int)x - 1;
        int y0 = y > 0.0 ? (int)y : (int)y - 1;
        int z0 = z > 0.0 ? (int)z : (int)z - 1;
        int x1 = x0 + 1, y1 = y0 + 1, z1 = z0 + 1;

        // Fractional offsets and smoothed interpolation weights.
        double fx = x - x0, fy = y - y0, fz = z - z0;
        double sx, sy, sz;
        switch (noiseQuality)
        {
            case NoiseQuality.Low: sx = fx; sy = fy; sz = fz; break;
            case NoiseQuality.Standard: sx = SCurve3(fx); sy = SCurve3(fy); sz = SCurve3(fz); break;
            default: sx = SCurve5(fx); sy = SCurve5(fy); sz = SCurve5(fz); break;
        }

        // Trilinear interpolation of the 8 corner gradient dot-products.
        // Bottom face (z = z0)
        double b00 = GradientNoise(x, y, z, x0, y0, z0, seed);
        double b10 = GradientNoise(x, y, z, x1, y0, z0, seed);
        double b01 = GradientNoise(x, y, z, x0, y1, z0, seed);
        double b11 = GradientNoise(x, y, z, x1, y1, z0, seed);
        double bottom = double.Lerp(
            double.Lerp(b00, b10, sx),
            double.Lerp(b01, b11, sx),
            sy);

        // Top face (z = z1)
        double t00 = GradientNoise(x, y, z, x0, y0, z1, seed);
        double t10 = GradientNoise(x, y, z, x1, y0, z1, seed);
        double t01 = GradientNoise(x, y, z, x0, y1, z1, seed);
        double t11 = GradientNoise(x, y, z, x1, y1, z1, seed);
        double top = double.Lerp(
            double.Lerp(t00, t10, sx),
            double.Lerp(t01, t11, sx),
            sy);

        return double.Lerp(bottom, top, sz);
    }

    // ── Gradient dot product ──────────────────────────────────────────────────

    /// <summary>
    /// Hashes the integer grid corner (<paramref name="ix"/>, <paramref name="iy"/>,
    /// <paramref name="iz"/>) and <paramref name="seed"/> to select a gradient
    /// vector from <see cref="RandomVectors"/>, then returns its dot product with
    /// the offset from that corner to the sample point
    /// (<paramref name="fx"/>, <paramref name="fy"/>, <paramref name="fz"/>).
    /// The result is scaled by 2.12 to normalise the output range to [-1, 1].
    /// </summary>
    private static double GradientNoise(
        double fx, double fy, double fz,
        int ix, int iy, int iz, long seed)
    {
        // Hash the corner coordinates and seed to a gradient table index.
        long hash = (1619L * ix + 31337L * iy + 6971L * iz + 1013L * seed) & 0xFFFFFFFFu;
        hash ^= hash >> 8;
        int vectorIndex = (int)(hash & 0xFF) << 2; // each entry is 4 doubles wide

        double gx = RandomVectors[vectorIndex];
        double gy = RandomVectors[vectorIndex + 1];
        double gz = RandomVectors[vectorIndex + 2];

        // Dot product of gradient with offset from corner to sample point.
        return (gx * (fx - ix) + gy * (fy - iy) + gz * (fz - iz)) * 2.12;
    }
}