...
public class SeekableStream : ResponseProducer
{
    ...
    private void ReadRangeHeader(HttpRequest request)
    {
        if (request.Headers.TryGetValue("Range", out string? rangeHeader))
        {
    ...
    {
        return $"bytes {startOffset}-{endOffset - 1}/{new FileInfo(file).Length}";
    }
