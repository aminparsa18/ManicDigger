...
/// </summary>
internal class HttpRequestReader : IDisposable
{
    /// <summary>
    ...
    /// </summary>
    private SocketAsyncEventArgs asyncArgs = null;
    ...
    /// </summary>
    private int requestBodyWritePosition = 0;

    /// <summary>
    /// The encoded request body, if any.
    /// </summary>
    private byte[] requestBodyEncoded = null;
    ...
    /// </summary>
    private byte[] recvOverflowBuffer = null;
    ...
    /// </summary>
    private ProcessingState processingState = ProcessingState.InitialLine;
    ...
    private void SockRecv(Socket sock)
    {
    ...
    private void ProcessRecv(object? sender, SocketAsyncEventArgs args)
    {
    ...
    private int PreProcessBuffer(SocketAsyncEventArgs args)
    {
        ...
        int headerEnd = IndexOf([13, 10, 13, 10], args.Buffer);
        if (headerEnd > -1) headerEnd += 4;
        else
        {
            headerEnd = IndexOf([10, 10], args.Buffer);
    ...
    /// <returns></returns>
    private static int IndexOf(byte[] pattern, byte[] data)
    {
        int ret = -1;
    ...
    {
        if (!int.TryParse(request.Headers["Content-Length"], out int contentLength) || contentLength > MaxBodySize)
        {
    ...
    private void ProcessRequestBody()
    {
        ...
        processingComplete = true;
        ReadComplete?.Invoke(request);
    }

    /// <summary>
    /// Processes the request text received so far.
    /// </summary>
    private void ProcessRequestText()
    {
        if (processingComplete)
            return;
            ...
            case ProcessingState.InitialLine:
                {
                    ...
                    if (lfIndex > -1)
                    {
                        var initialLine = requestText[..(lfIndex + 1)].Trim();
                        requestText = requestText[(lfIndex + 1)..];
            ...
            case ProcessingState.Headers:
                {
                    ...
                    while (lfIndex > -1)
                    {
                        var line = requestText[..(lfIndex + 1)].Trim();
                        if (line == "")
                        {
    ...
    private void ParseHeader(string line)
    {
        ...
        if (seperatorIndex > -1)
        {
            var header = line[..seperatorIndex];
            var value = line[(seperatorIndex + 1)..].Trim();
            if (!request.Headers.TryAdd(header, value))
    ...
    private void ParsingError(HttpStatusCode httpStatusCode)
    {
        processingComplete = true;
        HttpError?.Invoke(httpStatusCode, request);
    }
